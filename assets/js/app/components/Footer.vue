<template>
    ...
    <ul class="flex-flow-center mb-12">
        <li v-for="(link, icon) in getDataFooter.brands">
            <a :href="link" :aria-label="uppercaseFirstLetter(icon)" target="_blank" rel="noopener"
...
const { data } = await useAsyncData('footer', () => $fetch(API_URL + 'footer'))

if (data && data?.value) {
    const { value: { data: dataFooter } } = data
